...
def get_plant_conditions(user_input):
    """
    Returns the optimal conditions for a given plant name (or variety).
...
def suggest_adjustments(user_input, optimal):
    ...
    if not suggestions: suggestions.append("✅ Conditions are optimal!")
    return suggestions
...
def predict():
    try:
        if request.method == "POST":
            ...
            # If no inputs, return optimal only
